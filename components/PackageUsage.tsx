...
export function RequestList({
  ...
  return (
    <ul className="space-y-1.5 text-sm" data-testid="scope-requests">
      {requests.map((r) => (
        <li key={r.id} className="rounded-md border border-edge bg-surface-2 px-3 py-2" data-testid="scope-request" data-status={r.status}>
          <div className="flex flex-wrap items-center justify-between gap-2">
            <span className="min-w-0 flex-1 truncate">{r.text}</span>
            <span className="text-xs text-muted">{STATUS[r.status]}</span>
          </div>
          ...
          {r.status === "pending_client" && onDecide && (
            <div className="mt-2 flex flex-wrap gap-2">
              {actor === "client" ? (
                <>
                  <button type="button" onClick={() => onDecide(r.id, "approved")} className="rounded-md bg-accent px-3 py-1.5 text-xs font-medium text-accent-ink" data-testid="scope-approve">
                    {`Aprovar extra de ${fmtMoney(r.extraPrice, lang)}`}
                  </button>
                  <button type="button" onClick={() => onDecide(r.id, "declined")} className="rounded-md border border-edge px-3 py-1.5 text-xs" data-testid="scope-decline">
                    Cancelar
                  </button>
                </>
              ) : (
