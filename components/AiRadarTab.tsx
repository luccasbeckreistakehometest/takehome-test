    ...
    <div className="space-y-6" data-testid="ai-radar">
      <Card className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          {draft.map((q, i) => (
            <div key={i} className="flex items-center gap-2">
              ...
              <button type="button" aria-label="Remover pergunta" onClick={() => setDraft(draft.filter((_, j) => j !== i))} className="px-1 text-text-muted hover:text-negative">
                ×
              </button>
            </div>
          ))}
          <div className="flex flex-wrap gap-2">
            {draft.length < data.maxQuestions && (
              <button type="button" className="t3 text-text hover:underline" onClick={() => setDraft([...draft, ""])}>
                + Pergunta
              </button>
            )}
            {data.aiAvailable && (
              <button type="button" className="t3 text-text hover:underline" onClick={() => act("suggest")} disabled={busy !== ""} data-testid="radar-suggest">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {!data.canRun && data.nextRunAt && <span className="t5 text-text-muted" data-testid="radar-next">{`Próxima rodada a partir de ${dateBr(data.nextRunAt)}`}</span>}
          {!data.aiAvailable && <span className="t5 text-text-muted">A IA não está disponível agora. Abaixo, um exemplo do que o radar mostra.</span>}
        </div>
      </Card>
      ...
      {latest ? (
        ...
        <p className="rounded-md border border-dashed border-edge p-4 t3 text-text-muted" data-testid="radar-empty">
...
function RadarResult({ run, trend, sample = false }: { run: Run; trend: number | null; sample?: boolean }) {
  ...
  return (
    <div className="space-y-4" data-testid={sample ? "radar-sample" : "radar-result"}>
      ...
      <div className="grid gap-x-8 gap-y-5 border-y border-edge py-4 sm:grid-cols-3">
        ...
        <Card>
          ...
          <ul className="mt-1 space-y-0.5 t3">
            ...
            {s.competitors.length === 0 && <li className="text-text-muted">Cadastre concorrentes no briefing ou na estratégia.</li>}
          </ul>
        </Card>
      </div>
      <Card>
        <SectionTitle>Por que não você</SectionTitle>
        <ul className="list-disc space-y-1 pl-5 t3">
            ...
            <tbody className="divide-y divide-rule">
              {run.results.map((r) => (
                <tr key={r.question} data-testid="radar-row">
                  ...
                  <td className="py-1.5 pr-2 align-top text-text-muted">{r.brandsMentioned.map((b) => `${b.position}. ${b.name}`).join(" · ")}</td>
      ...
      <p className="t5 text-text-muted" data-testid="radar-disclaimer">
