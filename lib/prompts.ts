...
function agencySystem(client: Client): string {
  // Estilo da casa da agência DONA da marca (nunca o de outra agência).
  const houseStyle = getAgency(client.agencyId)?.houseStyle ?? "";
  const language =
    client.language === "en"
